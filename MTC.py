...
#####################TCM function##########################
        ...
        random_maintenance = 0  ###indicates if we want random maintenance or not (o = no, 1= yes)
        single_exp = 1
        print("-------------output------------")
        ...
        #lowest_energy = calc_lowest_required_energy(1,1,1) 
        #print("lowest energy required:", lowest_energy)
        ...
        def cost_matrix_weight_calc(k,i,h,remaining_obj_task_matrix, extra_robs, extra_rob_checked):
                ...
                for j in Non_Navigation_Tasks:
                    if remaining_obj_task_matrix[h,j_sort_ascending[j]] == 1:
                        ...
                        if tempp_e >= E_changeMax:
                            print("now energy is above echangmax----")
                            # if tempp_e < Edod and extra_robs > 0: ##########new addition
                            if extra_robs > 0: ##########new addition
    
                                print("extra robots are there so obj1 is 0")
                                obj1_rounded = 0
                                extra_rob_checked = 1
                            else:    
                            ...
                            obj2_rounded = temp_calc_obj2(tempp_e, e_waste)
                            total_obj = obj1_rounded + obj2_rounded
                            print(f"prev obj = {prev_obj}")
                            print(f"obj1={obj1_rounded}, obj2={obj2_rounded}, total is {total_obj}")
                            
                            if total_obj > prev_obj:
                                break
                            task_removed = temp_task_removed
                            print("task removed=", task_removed)
            ...
            #####----to handle obj changes---------------------###
                ...
                for j in Non_Navigation_Tasks:
                    if remaining_obj_task_matrix[h,j_sort_ascending[j]] == 1:
                        ...
                        obj2_rounded = temp_calc_obj2(tempp_e, e_waste)
                        print(f"energy after {temp_task_removed} temporary tasks removed is {tempp_e}")
                        total_obj = obj1_rounded + obj2_rounded
                        print(f"prev obj = {prev_obj}")
                        print(f"obj1={obj1_rounded}, obj2={obj2_rounded}, total is {total_obj}")
            ...
            print("final_obj", prev_obj)
            print("final total task removed:", task_removed)
            print("final allocated obj tasks", allocated_obj_task)       
            return prev_obj, allocated_obj_task, extra_rob_checked
        ...
        def nav_to_robot_assign(e, k, available_robots, available_nav_tasks, remaining_obj_task_matrix, total_e_of_avail_robs):
            print("----in nav_to_robot_assign function")
            ...
            disallowed_robots=[]
            disallowed_nav_tasks = []
            extra_robs = len(available_robots) - len(available_nav_tasks) 
            extra_rob_checked = 0
            print("extra robot before starting to build the matrix", extra_robs)
            ...
            for h in range(0, len(available_nav_tasks)):
                ...
                for i in range(0, len(available_robots)):
                    ...
                    weight, allocated_obj_task, extra_rob_checked = cost_matrix_weight_calc(k,available_robots[i], available_nav_tasks[h], remaining_obj_task_matrix, extra_robs, extra_rob_checked)
                    ###for multiplying weights by energy contribution####
                    if weight != -1 and (sum((Robot_Navigation_state_rounded[k - 1, available_robots[i], hh]) for hh in Navigation_Tasks)) > 0:
                        weight = weight * ((100 - e[k-1,available_robots[i]])/ total_e_of_avail_robs)
                        print("multiplication of energy contribution factor done!!")
                    ##new change end    
                    if weight == -1:
                    ...
                    temp_RNS[k,i,h] = 0
                    extra_robs = extra_robs - extra_rob_checked
                    extra_rob_checked = 0
                    print("extra robot checked", extra_rob_checked)
                    print("extra robs", extra_robs)

            # print("weight matrix before tuning", weight_matrix)
            ...
            ###to delete rows that has all disallowed#####    
            # row_index=[] 
            # for i in range(len(weight_matrix)): 
            #     flag = True 
            #     for j in range(len(weight_matrix[0])): 
            #         if weight_matrix[i][j] != DISALLOWED:
            #             flag = False
            #     if flag == True:
            #         row_index.append(i)     
            ...
            # for i in row_index:
            #     disallowed_nav_tasks.append(available_nav_tasks[i])
            
            # available_nav_tasks = [item for item in available_nav_tasks if item not in disallowed_nav_tasks]    
            # ##to dlete the rows with all disallowed 
            # iter1 = len(row_index)           
            # if iter1 > 0:
            #     weight_matrix  = np.delete(weight_matrix, row_index, 0).tolist()
            #     allocated_obj  = np.delete(allocated_obj, row_index, 0).tolist()   
                
        ##########--------------------------###########                 
            print("weight matrix after tuning", weight_matrix)  
            #####new change for multiplying 
            ####new addition to make equal weights 1 less####
            rob_index = [] ##to store the robot indexs in the weight matrix that has same weights for all the navs
            for j in range(len(weight_matrix[0])):
                flag2 = True
                first_value = weight_matrix[0][j]
                for i in range(len(weight_matrix)):
                    if weight_matrix[i][j] != first_value:
                        flag2 = False
                        break
                if flag2 == True:
                    rob_index.append(j)    
            print("col indexs in weight matrix that has equal weights in a whole column", rob_index)
            
            ###now to minimize the equal weights a lil bit for getting the previous allocation
            for i in rob_index:
                for h in range(0, len(weight_matrix)):
                    if Robot_Navigation_state_rounded[k-1, available_robots[i], available_nav_tasks[h]] == 1:
                        weight_matrix[h][i] = weight_matrix[h][i] - 0.01
            
            print("weight matrix after making same weights a lil different ", weight_matrix)
            #########new addition end
            result = munkres_func(weight_matrix)
            # print(f"result from munkres at k={k} is {result}")
            ...
            return task_robot_pairs, new_allocated_obj_task, disallowed_robots  #disallowed_nav_tasks  
        ...
        def allocation(k, available_robots, available_nav_tasks, need_charge_robots,
            ...
            v=0
            print("----in allocation function----")
            total_e_of_avail_robs = 0
            for i in available_robots:
                total_e_of_avail_robs += e[k-1,i]
            print("total energy of avail robos:", total_e_of_avail_robs)    
            while len(available_robots) != 0 and len(available_nav_tasks) != 0:
                v= v+1
                task_robot_pairs, new_allocated_obj_task, disallowed= nav_to_robot_assign(e, k, available_robots, available_nav_tasks, remaining_obj_task_matrix, total_e_of_avail_robs)
                print("task robot pair from munkres", task_robot_pairs)
                ...
                # available_nav_tasks = [item for item in available_nav_tasks if item not in disallowed_nav_tasks]     
                print("need charge robots after allocation:", need_charge_robots)    
                print("available robs:", available_robots)
                ...
                print("available robs after allocation:", available_robots)
                print(f"after first allocation at k= {k}, avail nav tasks {available_nav_tasks}") 
        ...
        #######functions to get robot list sorted from highest energy to lowest
        # def my_sort(a):
        #     b = dict(sorted(a.items(), key=lambda x: x[1], reverse=True))
        #     return list(b.keys())

        # def get_robots_sorted_by_energy(k,charging_robot_array, energy_array):
        #   mapped_dict = dict()
        #   for i in Robots:
        #     if i in charging_robot_array:
        #       mapped_dict[i] = energy_array[k,i]
        #   sorted_list = my_sort(mapped_dict)
        #   return sorted_list
        ...
        if random_maintenance == 0:
            ...
            print("calculating maint periods")
        ...
        if random_maintenance == 1:
            ("calculating random maintenance periods")
            maint_periods= {}
            for i in main_robot_list:
                maint_periods[i] = random.randint(0, (T- main_dur)+1)
            
            print("maintenance periods from RM:", maint_periods)
        ...
        ########______________________main MTC_______________############
        e = np.zeros((T, R))
        temp_e = np.zeros((T, R))
        ...
        for k in Times:
            ...
            ####now start of the next period
                ...
                avail_rob_dict = {}
                for i in available_robots:
                    avail_rob_dict[i] = e[k-1,i]
                # print("avail rob dict:", avail_rob_dict)
                available_robots = sorted(avail_rob_dict, key=avail_rob_dict.get, reverse=False)
                print("avail robos sorted:", available_robots)    
                ####new addition end
        ...
        if random_maintenance == 0:
            Setting_df.loc[Exp_no,"TCM_obj1"] = obj1_rounded
            Setting_df.loc[Exp_no,"TCM_obj2"] = obj2_rounded
            Setting_df.loc[Exp_no,"TCM_obj2_weighted"] = W_obj2_rounded
            Setting_df.loc[Exp_no,"TCM_total_cost"] = obj_rounded
            Setting_df.loc[Exp_no,"total_cost_LP"] = total_obj_LP
            Setting_df.loc[Exp_no,"TCM_Total_tasks"] = total_tasks_to_perform
            Setting_df.loc[Exp_no,"TCM_task_downtime"] = TCM_task_downtime
            Setting_df.loc[Exp_no,"TCM_TA%"] = (total_allocated_tasks/total_tasks_to_perform)
            Setting_df.loc[Exp_no,"TCM_run_time_with_LP"] = run_time_TCM
            Setting_df.loc[Exp_no,"TCM_run_time_without_LP"] = run_time_only_TCM
            Setting_df.loc[Exp_no,"TCM_EEF"] = EEF
            Setting_df.loc[Exp_no,"TCM_SoC_v"] = final_soc_violation
        ...
        ###for storing the other version TCM+randomized maintenance
            ...
            Setting_df.loc[Exp_no,"TCM_rand_main_obj1"] = obj1_rounded
            Setting_df.loc[Exp_no,"TCM_rand_main_obj2"] = obj2_rounded
            Setting_df.loc[Exp_no,"TCM_rand_main_obj2_weighted"] = W_obj2_rounded
            Setting_df.loc[Exp_no,"TCM_rand_main_total_cost"] = obj_rounded
            # Setting_df.loc[Exp_no,"total_cost_LP"] = total_obj_LP
            Setting_df.loc[Exp_no,"TCM_rand_main_Total_tasks"] = total_tasks_to_perform
            Setting_df.loc[Exp_no,"TCM_rand_main_task_downtime"] = TCM_task_downtime
            Setting_df.loc[Exp_no,"TCM_rand_main_TA%"] = (total_allocated_tasks/total_tasks_to_perform)

            Setting_df.loc[Exp_no,"TCM_rand_main_run_time"] = run_time_TCM
            Setting_df.loc[Exp_no,"TCM_rand_main_EEF"] = EEF
            Setting_df.loc[Exp_no,"TCM_rand_main_SoC_v"] = final_soc_violation
            Setting_df.loc[Exp_no,"run_time_LP"] = total_time_LP
            Setting_df.loc[Exp_no,"TCM"] = 0
...
#         plt.plot(range(0, T + 1), state_of_charge_a[:, 0], linestyle='-', label="Robot 0")  
#         plt.plot(range(0, T + 1), state_of_charge_a[:, 1], linestyle='--', label="Robot 1")
#         plt.plot(range(0, T + 1), state_of_charge_a[:, 2], linestyle='dotted', label="Robot 2")  
...
#         plt.legend(bbox_to_anchor=(0., -0.5, 1., -0.11), loc='upper left',
